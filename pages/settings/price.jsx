...
function AccountPrice(props) {
  ...
  useEffect(() => {
    let setFilteredPrices = async () => {
      ...
      prices.map((price) => {
        if (price.type === subscription.name) {
          price.tag = {
            tagged: false,
            text: "Current plan",
            primary: true,
      ...
      let _filteredPrices = prices.filter((price) => {
        if (subscription.name !== "free" && price.name === "free") {
          return false;
        }
        return true;
      });
  ...
  return (
    <AccPageWrapper title="Price" heading="Subscription">
        ...
        {filteredPrices.map((price) => (
          ...
          >
            ...
            >
              ...
              <Button
                className={cn(
                  "my-4 w-full bg-primary-2  text-primary-600",
                  price.type === subscription.name
                    ? "bg-accent-2 mb-0"
                    : "bg-primary-2"
                )}
                onClick={(e) => handleSelectPlan(price)}
              >
                {price.type === subscription.name ? "Current Plan" : "Upgrade"}
              ...
              {price.type === subscription.name ? (
                ...
                >
                  Plan Ends in {subscription.expires_in} days
                </Text>
              ) : null}
